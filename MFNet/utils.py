...
FOLDER = os.environ.get("SSRS_DATA_ROOT", "/root/SSRS/autodl-tmp/dataset")
if not FOLDER.endswith('/'):
    FOLDER += '/'
BATCH_SIZE = int(os.environ.get("SSRS_BATCH_SIZE", "10"))
...
if DATASET == 'Vaihingen':
    ...
    BOUNDARY_FOLDER = BOUNDARY_TEMPLATE_ENV or (MAIN_FOLDER + 'sam_boundary_merge/ISPRS_merge_{}.tif')
    OBJECT_FOLDER = OBJECT_TEMPLATE_ENV or (MAIN_FOLDER + 'V_merge/V_merge_{}.tif')
    ...
    BOUNDARY_FOLDER = BOUNDARY_TEMPLATE_ENV or (MAIN_FOLDER + 'sam_boundary_merge/ISPRS_merge_{}.tif')
    OBJECT_FOLDER = OBJECT_TEMPLATE_ENV or (MAIN_FOLDER + 'P_merge/P_merge_{}.tif')
elif DATASET == 'Hunan':
    ...
    BOUNDARY_FOLDER = BOUNDARY_TEMPLATE_ENV
    OBJECT_FOLDER = OBJECT_TEMPLATE_ENV
    palette = {
...
class ISPRS_dataset(torch.utils.data.Dataset):
    def __init__(self, ids, data_files=DATA_FOLDER, label_files=LABEL_FOLDER,
        ...
        self.boundary_files = [BOUNDARY_FOLDER.format(id) for id in ids] if BOUNDARY_FOLDER else [None] * len(ids)
        self.object_files = [OBJECT_FOLDER.format(id) for id in ids] if OBJECT_FOLDER else [None] * len(ids)
    ...
    def __len__(self):
        if DATASET == 'Potsdam' or DATASET == 'Vaihingen':
            epoch_steps = int(os.environ.get("SSRS_EPOCH_STEPS", "1000"))
            return BATCH_SIZE * epoch_steps
        elif  DATASET == 'Hunan':
            epoch_steps = int(os.environ.get("SSRS_EPOCH_STEPS", "500"))
            return BATCH_SIZE * epoch_steps
        else:
            return None
    ...
    def __getitem__(self, i):
        ...
        if random_idx in self.boundary_cache_.keys():
            ...
            boundary_path = self.boundary_files[random_idx]
            if STRUCTURE_SUPERVISION and boundary_path and os.path.isfile(boundary_path):
                boundary = np.asarray(io.imread(boundary_path), dtype='float32')
                if boundary.ndim == 3:
                    boundary = boundary[:, :, 0]
        ...
        if random_idx in self.object_cache_.keys():
            object_map = self.object_cache_[random_idx]
        else:
            object_path = self.object_files[random_idx]
            if STRUCTURE_SUPERVISION and object_path and os.path.isfile(object_path):
                object_map = np.asarray(io.imread(object_path), dtype='int64')
                if object_map.ndim == 3:
                    object_map = object_map[:, :, 0]
            else:
                object_map = np.zeros(label.shape, dtype=np.int64)
            if self.cache:
                self.object_cache_[random_idx] = object_map
...
def metrics_loveda(predictions, gts, label_values=LABELS, return_details=False):
    cm = confusion_matrix(
        gts,
        predictions,
        labels=range(len(label_values)))
