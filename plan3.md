...
- `fineNet/train_phase3_unet.py` 的 UNet decoder
- `fineNet/train_dual_stream.py` 的 ConvBlock + Up 模块
...
- **可快速验证**：复用 `fineNet/` 现有 decoder 代码，改动量最小
...
**代码：** `RS-SAM3-p3/` | **数据划分：** MFNet splits (gts_for_participants) | **评估协议：** 256² 滑动窗口
...
1. 在 `fineNet/` 中实现 `AdapterViT` 包装类
