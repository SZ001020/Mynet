...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        args = cfg.parse_args()
        sam_encoder = os.environ.get("SSRS_MFNET_SAM_ENCODER", "vit_l")
        sam_ckpt = os.environ.get("SSRS_MFNET_SAM_CKPT", "weights/sam_vit_l_0b3195.pth")

        # Fallback to the locally available SAM-H checkpoint in this repo.
        fallback_encoder = "vit_h"
        fallback_ckpt = "/root/SSRS/SAM_RS/sam_vit_h_4b8939.pth"
        if not os.path.isfile(sam_ckpt) and os.path.isfile(fallback_ckpt):
            sam_encoder = fallback_encoder
            sam_ckpt = fallback_ckpt

        self.sam = sam_model_registry[sam_encoder](args, checkpoint=sam_ckpt)
