...
Plan13 — 多层级 ViT 特征 + RGB 纹理分支（树/草区分）
  │
  ├─→ P13-A (多层级 ViT 特征金字塔, from scratch, nDSM) → 76.42% ❌ -0.30pp vs P11-B
  ├─→ P13-C (RGB 纹理分支 + 植被 refinement, from P11-B) → 76.70% ❌ -0.02pp vs P11-B
  ├─→ P13-E (shared building gate, from P13-C) → 76.54% ❌ -0.18pp vs P11-B
  └─→ P13-F (per-class gate + cross-suppression, from P13-C) → 76.64% ❌ -0.08pp vs P11-B
       P13-B (veg refinement head) 和 P13-D (gated nDSM) 因 P13-A/C 无效而被跳过
       MFNet 协议: P13-F 85.61% (+0.30pp), P13-C 85.55%, P13-E 85.53%, P13-A 84.84%
       结论: 全部噪声级。纹理分支是零和重排序工具, 非净提升。
...
| 多层级 ViT 特征金字塔 (block7/15/23/31) | P13-A 84.84 vs P11-B 85.31 (MFNet) | -0.47pp | Plan13 |
| RGB 纹理分支 (TextureStem + gate variants) | P13-C/E/F 全部 ≤0.3pp, 无共识改善, 零和重排序 | 噪声级 | Plan13 |
