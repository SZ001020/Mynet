...
> 方案 B (全量微调) 和 C (LoRA) 因 SAM 3 融合算子 (`perflib/fused.py`) 不支持梯度计算而无法运行。正确路径是使用 SAM 3 官方的 `sam3-main/sam3/train/train.py` + 更大数据集。
...
| 类型 | 路径 |
|---|---|
| 代码 | `SegEarth-OV-3-main/phase1_experiments.py` — 6 head × presence 实验运行器 |
| 代码 | `SegEarth-OV-3-main/phase1_visualize.py` — 推理图对比生成 |
| 代码 | `SegEarth-OV-3-main/segearthov3_segmentor.py` — SegearthOV3 模型封装（已修改） |
| 代码 | `SegEarth-OV-3-main/configs/cfg_*_noclutter*.py` — 5 类评估配置 |
...
| 类型 | 路径 |
|---|---|
| 代码 | `SegEarth-OV-3-main/phase2_prompt_engineering.py` — 4 组 prompt × 2 head 实验运行器 |
| 代码 | `SegEarth-OV-3-main/configs/cls_*_prompt_{b,c,d,e}.txt` — 各组 prompt 文本 |
| 代码 | `SegEarth-OV-3-main/configs/cfg_*_prompt_{b,c,d,e}.py` — 各组评估配置 |
...
| 类型 | 路径 |
|---|---|
| 代码 | `fineNet/train.py` — FPN decoder 训练（A1/A2/A3 三种数据策略） |
| 代码 | `fineNet/finetune_model.py` — FineTunedSAM3 + FPNHead + LoRALinear |
| 代码 | `fineNet/finetune_dataset.py` — 训练数据集（V+P 混合/单独） |
| 代码 | `fineNet/train_phase3_unet.py` — UNet decoder 训练（升级版） |
...
| 类型 | 路径 |
|---|---|
| 代码 | `fineNet/phase4_approach_a.py` — DSM logit bias 注入（基于 mmseg eval） |
| 代码 | `fineNet/phase4_dsm.py` — DSM 预处理 + 推理（独立评估器） |
| 代码 | `fineNet/train_phase4_dsm.py` — 双流 DSM 微调（RGB+DSM → decoder） |
