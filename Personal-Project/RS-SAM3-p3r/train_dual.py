...
Upgrades over p3:
1. UNetFormer decoder (GLA window attention) replacing simple UNet ConvBlocks
2. Deep DSM fusion at every ViT block (AdapterBlockDSM) instead of post-hoc concat

用法:
  cd /root/Mynet/RS-SAM3-p3r
  python train_dual.py --dataset vaihingen --epochs 20 --batch 4 --model dsm
  python train_dual.py --dataset vaihingen --epochs 20 --batch 4 --model rgb
"""
...
sys.path.insert(0, '/root/Mynet/Personal-Project/RS-SAM3-p3r')
...
def create_dsm_dataloaders(dataset_name, batch_size=4, crop_size=512, num_workers=0):
    ...
    train_ds = ISPRSTrainDatasetDSM(img_dir, gt_dir, train_tiles, img_suf, gt_suf,
                                    dsm_dir, dsm_pat, stem_fn, crop_size, is_train=True)
    val_ds = ISPRSTrainDatasetDSM(img_dir, gt_dir, val_tiles, img_suf, gt_suf,
    ...
    val_ldr = torch.utils.data.DataLoader(val_ds, batch_size=1, shuffle=False, num_workers=0, pin_memory=True)
...
def main():
    parser = argparse.ArgumentParser(description='Plan3 p3r: Deep DSM + UNetFormer Decoder')
    ...
    out_dir = os.path.join(args.output, f'plan3_p3r_{args.model}_{ts}')
    os.makedirs(out_dir, exist_ok=True)

    use_dsm = (args.model == 'dsm')
    print(f"Plan3 p3r: Deep DSM Fusion + UNetFormer Decoder")
    print(f"  Dataset: {args.dataset}, Model: {args.model}, Epochs: {args.epochs}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for bi, (images, dsm, labels) in enumerate(train_ldr):
            images, labels = images.to(device), labels.to(device)
            dsm_d = dsm.to(device) if use_dsm else None

            logits = model(images, dsm_d) if use_dsm else model(images)
            logits = F.interpolate(logits, labels.shape[-2:], mode='bilinear', align_corners=False)
