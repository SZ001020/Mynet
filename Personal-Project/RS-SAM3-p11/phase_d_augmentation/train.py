#!/usr/bin/env python3
"""P11-D: nDSM + veg boundary loss + photometric data augmentation (from scratch, seed=42)."""
...
P11D_DIR = f"{BASE}/Personal-Project/RS-SAM3-p11/phase_d_augmentation"
...
sys.path.insert(0, P11D_DIR)     # P11-D dataset_online (nDSM + augmentation)
sys.path.insert(0, P11A_DIR)     # P11-A structure_loss (veg_boundary_weight, HIGHEST priority)

from dataset_adapter import (  # noqa: E402
    IGNORE_INDEX, NUM_CLASSES,
    POTSDAM_TRAIN, POTSDAM_VAL, VAIHINGEN_TRAIN, VAIHINGEN_VAL, _rgb_to_class,
)
from dataset_online import OnlineCropDataset, compute_ndsm  # noqa: E402 (P11-D version)
from model import Plan7PromptMFNet  # noqa: E402
from structure_loss import structure_loss  # noqa: E402 (P11-A veg boundary version)
...
def main():
    ...
    parser.add_argument("--aug-color-jitter", type=float, default=0.2,
                        help="ColorJitter range for brightness/contrast/saturation (default 0.2)")
    parser.add_argument("--aug-blur-prob", type=float, default=0.3,
                        help="GaussianBlur probability (default 0.3)")
    ...
    out_dir = os.path.join(args.output, f"plan11_d_aug_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("P11-D: nDSM + veg boundary loss + data augmentation")
    print(f"  aug: color_jitter={args.aug_color_jitter}, blur_prob={args.aug_blur_prob}")
    ...
    train_ds = OnlineCropDataset(
        ...
        aug_color_jitter=args.aug_color_jitter, aug_blur_prob=args.aug_blur_prob,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (images, dsm, labels) in enumerate(train_loader):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                logits = model(images, dsm)
                logits = F.interpolate(logits, labels.shape[-2:], mode="bilinear", align_corners=False)
                loss = structure_loss(logits.float(), labels, veg_boundary_weight=args.veg_boundary_weight)
