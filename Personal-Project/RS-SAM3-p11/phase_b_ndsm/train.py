#!/usr/bin/env python3
"""P11-B: Train with nDSM + global normalization (from scratch, seed=42)."""
...
sys.path.insert(0, P11B_DIR)     # P11-B dataset_online (nDSM, HIGHEST priority)

from dataset_adapter import (  # noqa: E402
    IGNORE_INDEX, NUM_CLASSES,
    POTSDAM_TRAIN, POTSDAM_VAL, VAIHINGEN_TRAIN, VAIHINGEN_VAL, _rgb_to_class,
)
from dataset_online import OnlineCropDataset, compute_ndsm  # noqa: E402 (P11-B version)
from model import Plan7PromptMFNet  # noqa: E402
from structure_loss import structure_loss  # noqa: E402 (original, no veg weight)
...
def main():
    ...
    out_dir = os.path.join(args.output, f"plan11_b_ndsm_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("P11-B: nDSM + global normalization (from scratch, seed=42)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (images, dsm, labels) in enumerate(train_loader):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                logits = model(images, dsm)
                logits = F.interpolate(logits, labels.shape[-2:], mode="bilinear", align_corners=False)
                loss = structure_loss(logits.float(), labels)  # original loss, no veg weight
