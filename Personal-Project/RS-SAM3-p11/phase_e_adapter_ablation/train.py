#!/usr/bin/env python3
"""P11-E: Adapter parameter ablation (reduced bottleneck + selective block injection).

Trains from scratch with nDSM normalization and original structure_loss.
"""
...
P11E_DIR = f"{BASE}/Personal-Project/RS-SAM3-p11/phase_e_adapter_ablation"
P11B_DIR = f"{BASE}/Personal-Project/RS-SAM3-p11/phase_b_ndsm"
sys.path.insert(0, SE)
sys.path.insert(0, PHASE7_DIR)   # fallback (lowest priority)
sys.path.insert(0, P11B_DIR)     # P11-B dataset_online (nDSM)
sys.path.insert(0, P11E_DIR)     # P11-E model + mm_adapter_vit (block_indices, HIGHEST priority)

from dataset_adapter import (  # noqa: E402
    IGNORE_INDEX, NUM_CLASSES,
    POTSDAM_TRAIN, POTSDAM_VAL, VAIHINGEN_TRAIN, VAIHINGEN_VAL, _rgb_to_class,
)
from dataset_online import OnlineCropDataset, compute_ndsm  # noqa: E402 (P11-B nDSM)
from model import Plan7PromptMFNet  # noqa: E402 (P11-E version with block_indices)
from structure_loss import structure_loss  # noqa: E402 (original)

CLASS_NAMES = ["road", "building", "grass", "tree", "car"]

# SAM3 ViTDet global attention block indices
        ...
        for tile in tiles:
            ...
            for y in range(0, h - 128, stride):
                for x in range(0, w - 128, stride):
                    ...
                    if ph < 128 or pw < 128:
                        continue
                    patch = img[y:y2, x:x2]; label = gt[y:y2, x:x2]; dsm_p = dsm[y:y2, x:x2]
                    if ph < 256 or pw < 256:
                    ...
                    if (label == IGNORE_INDEX).mean() <= 0.5:
                        self.samples.append((patch, dsm_p, label))
...
def dataset_paths(dataset: str):
    if dataset == "vaihingen":
        ...
        dsm_paths = {t: f'{dsm_dir}/dsm_09cm_matching_area{t.replace("top_mosaic_09cm_area", "")}.tif' for t in tiles_train + tiles_val}
    else:
        ...
        dsm_paths = {t: f'{dsm_dir}/dsm_potsdam_{t.replace("top_potsdam_", "")}.tif' for t in tiles_train + tiles_val}
...
def load_sam3(device="cuda"):
    ...
    model = build_sam3_image_model(bpe_path=f"{SE}/sam3/assets/bpe_simple_vocab_16e6.txt.gz",
...
def main():
    ...
    parser.add_argument("--adapter-bottleneck", type=int, default=8,
                        help="Adapter bottleneck dim (default 8 for ablation, was 32)")
    parser.add_argument("--adapter-placement", default="global", choices=["all", "global"],
                        help="'all' = all 32 blocks, 'global' = global attention blocks only [7,15,23,31]")
    ...
    block_indices = GLOBAL_BLOCKS if args.adapter_placement == "global" else None
    ...
    out_dir = os.path.join(args.output, f"plan11_e_adapter_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print(f"P11-E: Adapter ablation (bottleneck={args.adapter_bottleneck}, placement={args.adapter_placement})")
    ...
    model = Plan7PromptMFNet(
        sam3, adapter_bottleneck=args.adapter_bottleneck,
        num_classes=NUM_CLASSES, dropout=0.1,
        dsm_attn_mode=args.dsm_attn_mode, checkpoint_attn=args.checkpoint_attn,
        resolution=args.resolution,
        block_indices=block_indices,
    ).cuda()
    model.train()

    prompt_params = list(model.prompt_encoder.parameters()); prompt_ids = {id(p) for p in prompt_params}
    dsm_params = list(model.dsm_encoder.parameters()); dsm_ids = {id(p) for p in dsm_params}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history["loss"].append(avg_loss); history["lr"].append(scheduler.get_last_lr())
