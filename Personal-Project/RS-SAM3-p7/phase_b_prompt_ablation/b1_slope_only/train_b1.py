#!/usr/bin/env python3
"""Train Plan7-B1: DSM slope-only prompt ablation."""
...
def main():
    ...
    out_dir = os.path.join(args.output, f"plan7_phase_b1_slope_only_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("Plan7-B1: DSM slope-only prompt")
