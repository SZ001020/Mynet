#!/usr/bin/env python3
"""Train Plan7-C3: SAM-HQ residual boundary correction on top of Plan7-A.

Adds GlobalLocalFusion + ResidualCorrectionHead to Plan7-A best checkpoint.
Only the new fusion/head params are trained; backbone + adapter + decoder frozen.
"""
...
def validate(model, loader, device):
    ...
    per_class_recall = {}
    for c in range(NUM_CLASSES):
        lc_sum = (labels == c).float().sum()
        per_class_recall[CLASS_NAMES[c]] = (inter[c] / lc_sum.clamp(min=1) * 100).item() if lc_sum > 0 else 0.0
    return {
...
def load_initial_checkpoint(model, path: str):
    ...
    print(f"  Missing keys: {len(missing)} (expected for C3 fusion/head)")
...
def main():
    ...
    parser.add_argument(
        "--init-from",
        default="/root/autodl-tmp/runs/plan7_phase_a_dsm_prompt_vaihingen_20260510_215044/best_model.pt",
    )
    ...
    out_dir = os.path.join(args.output, f"plan7_c3_residual_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("Plan7-C3: SAM-HQ residual boundary correction")
    ...
    # Freeze all existing parameters, only train C3 new components
    c3_param_names = {"hq_fusion", "residual_head"}
    for n, p in model.named_parameters():
        if not any(key in n for key in c3_param_names):
            p.requires_grad = False

    model.train()

    # Separate parameter groups for C3 components (higher lr for new modules)
    residual_params = (
        list(model.hq_fusion.parameters()) + list(model.residual_head.parameters())
    )
    residual_ids = {id(p) for p in residual_params}
    other_params = [p for p in model.parameters() if p.requires_grad and id(p) not in residual_ids]
    print(
        f"  Optim params: residual={sum(p.numel() for p in residual_params):,}, "
        f"other={sum(p.numel() for p in other_params):,}"
    )
    optimizer = torch.optim.AdamW(
        [
            {"params": residual_params, "lr": args.residual_lr},
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if metrics:
            ...
            history["metrics"].append(
                {"avg_oa": 0, "avg_miou": 0, "per_class_iou": {c: 0 for c in CLASS_NAMES}}
            )
        json.dump(history, open(os.path.join(out_dir, "history.json"), "w"), indent=2)

    print(f"\nDone. Best: epoch={best_metrics and 'see best_model.pt'}, mIoU={best_miou:.2f}%")
