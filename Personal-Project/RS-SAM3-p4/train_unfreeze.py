#!/usr/bin/env python3
"""
Plan4 D1-D3: 渐进解冻 SAM3 ViT 尾部 layer 微调

用法:
  cd /root/Mynet/RS-SAM3-p4
  python train_unfreeze.py --unfreeze 8 --model dsm --epochs 30 --batch 4   # D1
  python train_unfreeze.py --unfreeze 16 --model dsm --epochs 30 --batch 2  # D2
"""
...
class VPT_MFNetDecoder_Unfreeze(nn.Module):
    """VPT + MFNet Decoder with optional ViT layer unfreezing."""

    def __init__(self, sam3_model, adapter_bottleneck=32, num_classes=5,
                 use_dsm=True, dropout=0.1, unfreeze_layers=0, window_size=8):
        super().__init__()
        self.backbone = sam3_model.backbone
        self.use_dsm = use_dsm

        # VPT Adapters (inject first so all blocks are wrapped)
        ...
        for n, p in self.backbone.vision_backbone.named_parameters():
            p.requires_grad = 'prompt_learn' in n  # only VPT trainable

        # Unfreeze last N ViT blocks
        vb = self.backbone.vision_backbone
        total_blocks = len(vb.trunk.blocks)
        if unfreeze_layers > 0:
            start = total_blocks - unfreeze_layers
            for i in range(start, total_blocks):
                blk = vb.trunk.blocks[i]
                inner = blk.block if hasattr(blk, 'block') else blk
                for p in inner.parameters():
                    p.requires_grad = True
            unfrozen = sum(p.numel() for p in vb.parameters() if p.requires_grad)
            print(f"  Unfroze blocks {start}-{total_blocks-1} ({unfrozen:,} params)")
...
def main():
    ...
    parser.add_argument('--unfreeze', type=int, default=8, help='Unfreeze last N ViT blocks (0=frozen, 8=D1, 16=D2, 32=D3)')
    parser.add_argument('--epochs', type=int, default=30)
    parser.add_argument('--batch', type=int, default=4)
    parser.add_argument('--lr_vit', type=float, default=1e-5, help='LR for unfrozen ViT layers')
    parser.add_argument('--lr', type=float, default=1e-4, help='LR for adapter+decoder')
    ...
    out_dir = os.path.join(args.output, f'plan4_d{args.unfreeze}_{args.model}_{ts}')
    os.makedirs(out_dir, exist_ok=True)

    use_dsm = (args.model == 'dsm')
    print(f"Plan4 D{args.unfreeze // 8}: Unfreeze {args.unfreeze} ViT blocks + MFNet Decoder")
    print(f"  Dataset: {args.dataset}, Epochs: {args.epochs}")
    print(f"  LR: ViT={args.lr_vit}, Adapter+Decoder={args.lr}")
    ...
    model = VPT_MFNetDecoder_Unfreeze(sam3, adapter_bottleneck=32, num_classes=NUM_CLASSES,
                                      use_dsm=use_dsm, dropout=0.1,
                                      unfreeze_layers=args.unfreeze).cuda()
    model.train()
    n_vit = sum(p.numel() for p in model.backbone.vision_backbone.parameters() if p.requires_grad)
    n_other = sum(p.numel() for p in model.parameters() if p.requires_grad) - n_vit
    print(f"  Trainable: ViT={n_vit:,} + Adapter/Decoder={n_other:,}")
    print(f"  LR: ViT={args.lr_vit}, Adapter/Decoder={args.lr}")

    # Layered LR: ViT unfrozen layers get lower LR
    vit_params = [p for n, p in model.named_parameters()
                  if p.requires_grad and 'prompt_learn' not in n
                  and 'decoder' not in n and 'dsm' not in n and 'fusion' not in n
                  and 'pyramid' not in n]
    adapter_params = [p for n, p in model.named_parameters()
                      if p.requires_grad and ('prompt_learn' in n or 'decoder' in n
                         or 'dsm' in n or 'fusion' in n or 'pyramid' in n)]
