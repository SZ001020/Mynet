"""P11-B: OnlineCropDataset with nDSM normalization.

Replaces per-tile min-max DSM normalization with morphological grey_opening-based
nDSM = DSM - ground, then clipped to [0, 10m] / 10m -> [0, 1].
"""
...
def compute_ndsm(dsm: np.ndarray, kernel_size: int = 101) -> np.ndarray:
    """Compute nDSM: subtract estimated ground (morphological opening) from DSM."""
    ...
    def __init__(
        ...
        for tile in tiles:
            ...
            if dp and os.path.exists(dp):
                ...
                dsm = compute_ndsm(dsm_raw)  # P11-B: nDSM normalization
            else:
