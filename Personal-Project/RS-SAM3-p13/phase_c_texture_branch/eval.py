#!/usr/bin/env python3
"""P13-C 256x256 sliding window evaluation with full confusion matrix."""
...
def predict_patch(model, rgb: np.ndarray, dsm: np.ndarray) -> np.ndarray:
    ...
    with torch.amp.autocast("cuda", dtype=torch.bfloat16):
        refined, _veg_delta = model(rgb_t.cuda(non_blocking=True), dsm_t.cuda(non_blocking=True))
...
def main():
    ...
    print(f"P13-C eval: {args.dataset}")
    ...
    print("SUMMARY")
    print("=" * 60)
    print(f"  OA={avg_oa:.2f}%  mIoU={avg_miou:.2f}%  mRecall={float(np.mean(list(per_class_recall.values()))):.2f}%")
    ...
    out = args.output or os.path.join(os.path.dirname(args.checkpoint), "eval_256_vaihingen_plan13_c.json")
