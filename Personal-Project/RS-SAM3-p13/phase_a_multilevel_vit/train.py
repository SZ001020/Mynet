#!/usr/bin/env python3
"""P13-A: Multi-level ViT feature pyramid (from scratch, seed=42, nDSM).

Routes ViT intermediate features (blocks 7, 15, 23, 31) to corresponding decoder
scales instead of creating all scales from a single deep feature via ConvTranspose.
"""
...
def main():
    ...
    out_dir = os.path.join(args.output, f"plan13_a_multilevel_vit_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("P13-A: Multi-level ViT feature pyramid (from scratch, seed=42, nDSM)")
