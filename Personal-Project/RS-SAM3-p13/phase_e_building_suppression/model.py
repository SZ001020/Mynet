"""P13-E model: P13-C + gated vegetation delta + building suppression.

Key change from P13-C: the vegetation refinement head now outputs a 3rd channel
(veg confidence gate) that is passed through sigmoid. The tree/grass delta is
multiplied by this gate before being added to the main logits. An anti-building
loss trains the gate to be 0 on building pixels, directly addressing the
grass->building confusion that offset P13-C's tree->grass improvements.
"""
...
class GatedVegetationRefinementHead(nn.Module):
    """Fuses texture + semantic features, outputs gated tree/grass delta.

    Outputs 3 channels: [tree_delta, grass_delta, veg_confidence_logit].
    The gate (sigmoid of ch2) modulates the delta to suppress vegetation
    predictions on non-vegetation surfaces (especially building).
    """
    ...
    def __init__(self, in_channels: int = 512, mid_channels: int = 128):
        ...
        self.fusion = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(mid_channels // 2, 3, 1),  # tree, grass, gate
        )

    def forward(self, semantic_feat, texture_feat):
        """Returns (gated_tree_grass_delta, gate_map).
        ...
        Returns:
            delta_gated: [B, 2, H, W] gated tree/grass delta logits
            gate_logit: [B, 1, H, W] raw gate logit (for BCEWithLogitsLoss)
        """
        fused = torch.cat([semantic_feat, texture_feat], dim=1)
        out = self.fusion(fused)  # [B, 3, H, W]
        raw_delta = out[:, :2]     # tree, grass delta
        gate_logit = out[:, 2:3]   # raw gate logit
        gate = torch.sigmoid(gate_logit)
        return raw_delta * gate, gate_logit


class Plan13EGatedMFNet(nn.Module):
    def __init__(
        self,
        sam3_model,
        ...
        self.veg_refine_head = GatedVegetationRefinementHead(in_channels=512, mid_channels=128)
        ...
        print(f"  Plan13-E model trainable: {trainable:,} / {total:,} ({trainable / total * 100:.2f}%)")
    ...
    def forward(self, images: torch.Tensor, dsm: torch.Tensor):
        ...
        veg_delta, veg_gate = self.veg_refine_head(feats[0], texture_feat)

        refined = decoder_out.clone()
        refined[:, 2:4] = refined[:, 2:4] + veg_delta  # grass=ch2, tree=ch3

        return refined, veg_delta, veg_gate
