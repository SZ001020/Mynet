#!/usr/bin/env python3
"""P13-E 256x256 sliding window evaluation."""
...
from model import Plan13EGatedMFNet
from train import dataset_paths, load_sam3
...
def predict_patch(model, rgb: np.ndarray, dsm: np.ndarray) -> np.ndarray:
    ...
    with torch.amp.autocast("cuda", dtype=torch.bfloat16):
        refined, _veg_delta, _gate_logit = model(rgb_t.cuda(non_blocking=True), dsm_t.cuda(non_blocking=True))
        logits = F.interpolate(refined, (256, 256), mode="bilinear", align_corners=False)
    return logits.argmax(1)[0].cpu().numpy()


def sliding_predict(model, rgb, dsm, window=256, stride=128):
    h, w = rgb.shape[:2]
...
def evaluate_tile(model, tile, img_dir, gt_dir, img_suffix, gt_suffix, dsm_path, no_confusion=False, use_ndsm=False):
    ...
    result = {"tile": tile, "oa": oa, "miou": miou, "correct": correct, "total": total,
...
def main():
    ...
    print(f"P13-E eval: {args.dataset}")
    ...
    results = [evaluate_tile(model, t, img_dir, gt_dir, img_suffix, gt_suffix, dsm_paths[t],
                             no_confusion=args.no_confusion, use_ndsm=args.ndsm) for t in tiles]
    ...
    summary = {
        ...
        "checkpoint_epoch": ckpt.get("epoch"), "checkpoint_crop_best": ckpt.get("best_v"),
    }

    if not args.no_confusion and any("confusion_matrix" in r for r in results):
        global_cm = sum(np.array(r["confusion_matrix"]) for r in results if "confusion_matrix" in r)
        print_confusion_matrix(global_cm, "CONFUSION MATRIX (raw counts)", CLASS_NAMES, CLASS_NAMES, "raw")
        print_confusion_matrix(global_cm, "ROW-NORMALIZED (Recall)", CLASS_NAMES, CLASS_NAMES, "recall")
        print_confusion_matrix(global_cm, "COL-NORMALIZED (Precision)", CLASS_NAMES, CLASS_NAMES, "precision")
        ...
        for name in CLASS_NAMES:
            f1 = float(2 * per_class_recall[name] * per_class_precision[name] /
                       max(per_class_recall[name] + per_class_precision[name], 1e-8))
            print(f"  {name:>12s}  {per_class_iou[name]:6.2f}%  {per_class_recall[name]:6.2f}%  "
                  f"{per_class_precision[name]:8.2f}%  {f1:6.2f}%")
    ...
    out = args.output or os.path.join(os.path.dirname(args.checkpoint), "eval_256_vaihingen_plan13_e.json")
