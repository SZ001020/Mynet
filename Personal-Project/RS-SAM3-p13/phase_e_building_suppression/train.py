#!/usr/bin/env python3
"""P13-E: Gated vegetation delta + building suppression.

Continues from P13-C checkpoint. Adds a vegetation confidence gate (sigmoid)
that the refinement head learns to set to 0 on building pixels, directly
addressing the grass->building confusion that offset P13-C's tree gains.
"""
...
P13E_DIR = f"{BASE}/Personal-Project/RS-SAM3-p13/phase_e_building_suppression"
sys.path.insert(0, SE)
sys.path.insert(0, P7_DIR)
sys.path.insert(0, P11B_DIR)
sys.path.insert(0, P13E_DIR)
...
from model import Plan13EGatedMFNet  # noqa: E402
...
def veg_refine_loss(refined_logits, labels, weight=1.0):
    ...
    veg_labels_1d = labels[veg_gt] - 2
    return weight * F.cross_entropy(veg_logits_2d, veg_labels_1d)


def building_suppression_loss(gate_logit, labels, weight=1.0):
    """BCEWithLogits loss: gate should be 0 on building pixels."""
    building_gt = (labels == 1)
    if building_gt.sum() < 1:
        return torch.tensor(0.0, device=labels.device)
    gate = F.interpolate(gate_logit.float(), labels.shape[-2:], mode="bilinear", align_corners=False)
    gate_building = gate.permute(0, 2, 3, 1)[building_gt]
    target = torch.zeros_like(gate_building)
    return weight * F.binary_cross_entropy_with_logits(gate_building, target)
...
def validate(model, loader, device):
    ...
    for images, dsm, labels in loader:
        ...
        refined, _veg_delta, _gate_logit = model(images, dsm)
...
def main():
    ...
    parser.add_argument("--new-lr", type=float, default=2e-5, help="LR for texture stem + refinement head")
    parser.add_argument("--veg-weight", type=float, default=0.5, help="Weight for vegetation refinement loss")
    parser.add_argument("--building-weight", type=float, default=0.3, help="Weight for building suppression loss")
    ...
    out_dir = os.path.join(args.output, f"plan13_e_building_suppression_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2, default=str)

    print("P13-E: Gated vegetation delta + building suppression")
    ...
    # Load P13-C checkpoint (fix 2ch -> 3ch head shape mismatch)
    if os.path.exists(args.init_from):
        ckpt = torch.load(args.init_from, map_location="cuda", weights_only=False)
        old_sd = ckpt["model"]

        # Fix: old head output 2ch (tree/grass), new head outputs 3ch (tree/grass/gate)
        old_w = old_sd.get('veg_refine_head.fusion.6.weight')
        old_b = old_sd.get('veg_refine_head.fusion.6.bias')
        if old_w is not None and old_w.shape[0] == 2:
            new_sd = model.state_dict()
            new_w = new_sd['veg_refine_head.fusion.6.weight'].clone()
            new_b = new_sd['veg_refine_head.fusion.6.bias'].clone()
            new_w[:2] = old_w
            new_b[:2] = old_b
            old_sd['veg_refine_head.fusion.6.weight'] = new_w
            old_sd['veg_refine_head.fusion.6.bias'] = new_b

        missing, unexpected = model.load_state_dict(old_sd, strict=False)
        print(f"  Loaded P13-C checkpoint (epoch {ckpt.get('epoch')}, mIoU={ckpt.get('best_v', 0):.2f}%)")
    ...
    history = {"loss": [], "loss_main": [], "loss_veg": [], "loss_building": [], "metrics": [], "lr": []}
    best_miou = 0.0
    best_metrics = None
    for epoch in range(1, args.epochs + 1):
        model.train()
        start = time.time()
        running_loss = 0.0; running_main = 0.0; running_veg = 0.0; running_building = 0.0
        for step, (images, dsm, labels) in enumerate(train_loader):
            images, dsm, labels = images.to(device), dsm.to(device), labels.to(device)
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                refined, _veg_delta, gate_logit = model(images, dsm)
                ...
                loss_building = building_suppression_loss(gate_logit, labels, weight=args.building_weight)
                loss = loss_main + loss_veg + loss_building
            ...
            running_building += loss_building.item()
            if step % 100 == 0:
                print(f"  E{epoch:03d}/{args.epochs} B{step:04d}/{len(train_loader)} "
                      ...
                      f"bld={loss_building.item():.4f}) lr={scheduler.get_last_lr()[0]:.2e}")

        scheduler.step()
        avg_loss = running_loss / max(len(train_loader), 1)
        avg_main = running_main / max(len(train_loader), 1)
        avg_veg = running_veg / max(len(train_loader), 1)
        avg_building = running_building / max(len(train_loader), 1)
        metrics = None
        if epoch == 1 or epoch % args.val_every == 0 or epoch == args.epochs:
            ...
            print(f"  E{epoch:03d}: loss={avg_loss:.4f} (main={avg_main:.4f} veg={avg_veg:.4f} "
                  f"bld={avg_building:.4f}) OA={metrics['avg_oa']:.2f}% mIoU={metrics['avg_miou']:.2f}% "
        ...
        history["loss_building"].append(avg_building)
