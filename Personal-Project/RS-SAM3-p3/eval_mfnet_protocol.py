...
from adapter_unet import AdapterSAM3UNet, AdapterSAM3UNetDSM
...
def load_rgb_model(ckpt_path, dropout=0.1):
    ...
    os.chdir(_prev)
    model = AdapterSAM3UNet(sam3, adapter_bottleneck=32, num_classes=5, dropout=dropout).cuda()
...
def load_dsm_model(ckpt_path):
    ...
    sam3 = build_sam3_image_model(
        ...
        checkpoint_path=f'{SE}/weights/sam3/sam3.pt', device='cuda')
    os.chdir(_prev)
    model = AdapterSAM3UNetDSM(sam3, adapter_bottleneck=32, num_classes=5, dropout=0.1).cuda()
    model.resolution = 1008
    ckpt = torch.load(ckpt_path, map_location='cuda', weights_only=False)
    model.load_state_dict(ckpt['model'], strict=False)
    model.eval()
...
def evaluate_tile(model, tile, img_dir, gt_dir, dsm_dir, img_suf, gt_suf, dsm_stem_fn, use_dsm):
    ...
    for c in range(5):
        ...
        tn = ((~pc) & (~lc) & mask).sum()
        oas[CLASS_NAMES[c]] = (inter + tn) / mask.sum() * 100
