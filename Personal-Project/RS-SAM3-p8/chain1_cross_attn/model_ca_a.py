"""Plan8 CA-A model: SAM3 + RGB/DSM/prompt MMAdapter w/ DSM->RGB cross-attn + MFNet decoder."""
...
from mm_adapter_vit_ca import DSMTokenEncoder, inject_mm_adapters  # noqa: E402


class Plan8CAMFNet(nn.Module):
    """Plan8 Chain 1 Phase A: in-ViT cross-attention DSM->RGB with MFNet decoder.

    Architecture = Plan7PromptMFNet + CrossAttentionAdapter at global blocks.
    """
    ...
    def __init__(
        ...
        print(f"  Plan8-CA model trainable: {trainable:,} / {total:,} ({trainable / total * 100:.2f}%)")
