#!/usr/bin/env python3
"""Train Plan8 CA-A: CTRL + unidirectional DSM->RGB cross-attention at global blocks."""
...
P8_DIR = f"{BASE}/Personal-Project/RS-SAM3-p8/chain1_cross_attn"
...
from model_ca_a import Plan8CAMFNet  # noqa: E402
...
def main():
    ...
    out_dir = os.path.join(args.output, f"plan8_ca_a_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("Plan8 CA-A: CTRL + unidirectional DSM->RGB cross-attention")
    print(f"  Dataset: {args.dataset}")
    print(f"  Output: {out_dir}")
    print(f"  Seed: {SEED}")
    print(f"  Cross-attn blocks: [7, 15, 23, 31]")
