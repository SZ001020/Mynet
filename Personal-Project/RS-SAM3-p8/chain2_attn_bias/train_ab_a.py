#!/usr/bin/env python3
"""Train Plan8 AB-A: CTRL + factorized DSM elevation attention bias at global blocks."""
...
P8_DIR = f"{BASE}/Personal-Project/RS-SAM3-p8/chain2_attn_bias"
...
from model_ab_a import Plan8ABMFNet  # noqa: E402
...
def main():
    ...
    out_dir = os.path.join(args.output, f"plan8_ab_a_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("Plan8 AB-A: CTRL + factorized DSM elevation attention bias")
    print(f"  Dataset: {args.dataset}")
    print(f"  Output: {out_dir}")
    print(f"  Seed: {SEED}")
    print(f"  Attn bias blocks: [7, 15, 23, 31]")
