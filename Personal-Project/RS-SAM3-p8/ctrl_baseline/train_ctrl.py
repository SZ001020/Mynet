#!/usr/bin/env python3
"""Train CTRL baseline: Plan7-A architecture, SAM3 base init, seed=42."""
...
# Seed must be set before any other random operations
SEED = 42
random.seed(SEED)
np.random.seed(SEED)
...
P8_DIR = f"{BASE}/Personal-Project/RS-SAM3-p8/ctrl_baseline"
sys.path.insert(0, SE)
sys.path.insert(0, PHASE_DIR)
sys.path.insert(0, P8_DIR)
...
from model_ctrl import Plan7PromptMFNet  # noqa: E402
...
def main():
    ...
    parser.add_argument("--init-from", default=None, help="No init-from for CTRL (SAM3 base)")
    ...
    out_dir = os.path.join(args.output, f"plan8_ctrl_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("Plan8 CTRL: gate-only (Plan7-A architecture, SAM3 base init, seed=42)")
    ...
    train_ds = OnlineCropDataset(
        ...
        is_train=True, crop_size=512, epoch_steps=args.epoch_steps, batch_size=args.batch,
    )
