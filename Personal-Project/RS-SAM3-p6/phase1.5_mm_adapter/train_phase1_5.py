#!/usr/bin/env python3
"""Train Plan6 Phase 1.5: online crops + LoRA/MMAdapter cooperation."""
...
class Window256DatasetDSM(torch.utils.data.Dataset):
    """Fixed validation windows to keep Phase 1 and Phase 1.5 comparable."""
...
def load_initial_checkpoint(model, path: str | None):
    if not path:
        return
    ckpt = torch.load(path, map_location="cpu", weights_only=False)
    missing, unexpected = model.load_state_dict(ckpt["model"], strict=False)
    print(f"  Initialized from: {path}")
    print(f"  Missing keys: {len(missing)} (expected for new LoRA params)")
...
def main():
    ...
    out_dir = os.path.join(args.output, f"plan6_phase1_5_mm_adapter_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("Plan6 Phase 1.5: online crops + LoRA/MMAdapter")
    ...
    load_initial_checkpoint(model, args.init_from or None)
    model.train()
    ...
    adapter_params = [
        ...
        if p.requires_grad and id(p) not in dsm_param_ids and id(p) not in lora_param_ids
    ]
    print(f"  Optim params: adapter={sum(p.numel() for p in adapter_params):,}, "
          f"dsm={sum(p.numel() for p in dsm_params):,}, lora={sum(p.numel() for p in lora_params):,}")
