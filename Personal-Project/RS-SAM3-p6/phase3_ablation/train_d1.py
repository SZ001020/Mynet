#!/usr/bin/env python3
"""D1 ablation: A3 architecture + CrossEntropy loss (vs structure_loss)"""
...
def make_online_ds(epoch_steps=1000,batch_size=2):
    ...
    return OnlineCropDataset("/root/autodl-tmp/dataset/Vaihingen/top","/root/autodl-tmp/dataset/Vaihingen/gts_for_participants",
                             tiles,".tif",".tif",dsm_paths,is_train=True,crop_size=256,epoch_steps=epoch_steps,batch_size=batch_size)
...
def make_val_ds():
    ...
    samples=[]
    for tile in tiles:
        ip=f"{img_dir}/{tile}.tif"; gp=f"{gt_dir}/{tile}.tif"; dp=dsm_paths[tile]
...
def main():
    ...
    out_dir=os.path.join(args.output,f"plan6_phase3_d1_{ts}")
    os.makedirs(out_dir,exist_ok=True)
    print(f"D1: CE loss, {args.epochs} epochs")
    train_ds=make_online_ds(epoch_steps=1000,batch_size=args.batch)
    val_ds=make_val_ds()
    ...
    for epoch in range(1,args.epochs+1):
        ...
        for step,(images,dsm,labels) in enumerate(train_loader):
            ...
            with torch.amp.autocast("cuda",dtype=torch.bfloat16):
                logits=model(images,dsm)
                logits=F.interpolate(logits,labels.shape[-2:],mode="bilinear",align_corners=False)
                loss=F.cross_entropy(logits.float(),labels,ignore_index=IGNORE_INDEX)
        ...
        print(f"  D1 E{epoch:02d}: loss={avg_loss:.4f} OA={metrics['avg_oa']:.2f}% mIoU={metrics['avg_miou']:.2f}%")
    ...
    print(f"D1 done. best={best_miou:.2f}%")
