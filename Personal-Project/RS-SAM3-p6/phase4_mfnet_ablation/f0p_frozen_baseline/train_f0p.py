#!/usr/bin/env python3
"""Train F0': Frozen SAM3 + 4xSEFusion + MFNetDecoder (MFNet "Without Adapter" equivalent).

Usage:
  F0':  python train_f0p.py --dataset vaihingen --epochs 12 --seed 42
"""
...
sys.path.extend([SE, PHASE1, F0P_DIR, SHARED])

from dataset_adapter import (  # noqa: E402
    IGNORE_INDEX, NUM_CLASSES, VAIHINGEN_TRAIN, VAIHINGEN_VAL, POTSDAM_TRAIN, POTSDAM_VAL, _rgb_to_class)
from model_f0p import FrozenSAM3DFM  # noqa: E402
...
def main():
    ...
    out_dir = os.path.join(args.output, f"plan6_phase4_f0p_frozen_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print(f"Plan6 Phase4 F0': Frozen SAM3 + 4xSEFusion (DFM) + MFNetDecoder")
