#!/usr/bin/env python3
"""Train F0'+L: F0' + LoRA (rank=8). Init from F0' best, 8 epochs."""
...
def main():
    ...
    out_dir = os.path.join(args.output, f"plan6_phase4_f0p_lora_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print(f"Plan6 Phase4 F0'+L: Frozen SAM3 + 4xSEFusion + LoRA (from F0' best)")
