"""F0'+L2: LoRA on attn q,v ONLY (no MLP LoRA), matching MFNet MMLoRA injection position."""
...
class FrozenSAM3DFMLoRAv2(nn.Module):
    """F0' + LoRA (attn q,v only) + 4xSEFusion + MFNetDecoder.

    LoRA only on attention q,v (no MLP), matching MFNet MMLoRA injection.
    """
    def __init__(self, sam3_model, num_classes=5, decode_channels=64,
        ...
        # LoRA only on attn qkv and proj (no MLP)
        ...
        print(f"  LoRA (attn-only) injected: {n} layers (rank={lora_rank})")
        ...
        print(f"  F0'+L2 model: trainable={trainable:,} / {total:,} ({trainable/ total*100:.2f}%)")
