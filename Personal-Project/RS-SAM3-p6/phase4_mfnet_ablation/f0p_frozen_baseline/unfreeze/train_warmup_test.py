#!/usr/bin/env python3
"""Test: F0'+L continuation with warmup scheduler (per-step linear warmup + cosine decay).

Hypothesis: Continuation stagnation (E1=peak) is caused by AdamW v_t=0 causing
large effective LR in first steps. Warmup prevents this without needing optimizer state.
"""
...
class OnlineCropDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (torch.from_numpy(patch.copy()).permute(2,0,1).float()/255.0,
                torch.from_numpy(dsm_p.copy()).float(), torch.from_numpy(label.copy()).long())
...
def main():
    ...
    parser.add_argument("--warmup-ratio", type=float, default=0.2, help="Fraction of total steps for warmup")
    ...
    out_dir = os.path.join(args.output, f"plan6_phase4_f0p_lora_warmup_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print(f"F0'+L warmup test: {args.warmup_ratio*100:.0f}% warmup, lr={args.lr:.1e}")
    print(f"  Init from F0' parent (4 epoch, no opt state)")
    ...
    print("\nBuilding model...")
    sam3 = load_sam3()
    model = FrozenSAM3DFMLoRA(sam3, num_classes=NUM_CLASSES, dropout=0.1, resolution=args.resolution).cuda()

    CKPT = "/root/autodl-tmp/runs/plan6_phase4_f0p_frozen_vaihingen_20260529_173409/best_model.pt"
    ...
    # Warmup scheduler: linear ramp-up → cosine decay, per-step
    ...
    if warmup_steps > 0:
        ...
        sch = SequentialLR(opt, schedulers=[warmup, cosine], milestones=[warmup_steps])
        print(f"  Warmup: {warmup_steps} steps ({args.warmup_ratio*100:.0f}%), per-step scheduler")
    else:
        sch = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=args.epochs)
        print(f"  No warmup (per-epoch scheduler)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (images, dsm, labels) in enumerate(train_loader):
            ...
            # Per-step scheduler
            if warmup_steps > 0: sch.step()
            if step % 100 == 0:
                print(f"  E{epoch:03d} B{step:04d} loss={loss.item():.4f} lr={opt.param_groups[0]['lr']:.2e}")

        if warmup_steps == 0: sch.step()  # Per-epoch for no-warmup
    ...
    print(f"\nDone. Best={best_miou:.2f}% | Run: {out_dir}")
