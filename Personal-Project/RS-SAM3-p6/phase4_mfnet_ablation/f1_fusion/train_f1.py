#!/usr/bin/env python3
"""Train F1/F2/F3: MMAdapter + optional LoRA + optional prompt.

Usage:
  F1: --use-lora --epochs 8 --init-from <F0_best>
  F2: --epochs 8 --init-from <F1_best>
  F3: --use-prompt --epochs 8 --init-from <F2_best>
"""
...
from model_f1 import Phase4AdapterModel  # noqa: E402
...
def load_checkpoint(model, path: str):
    ...
    print(f"  Loaded from {os.path.basename(path)}: missing={len(missing)}, unexpected={len(unexpected)}")
...
def main():
    ...
    tags = []
    if args.use_lora: tags.append("lora")
    if args.use_prompt: tags.append("prompt")
    tag = "_".join(tags) if tags else "frozen"
    ...
    print(f"Plan6 Phase4 ({tag}): {'+LoRA' if args.use_lora else ''}{'+prompt' if args.use_prompt else ''}")
    ...
    print("\nBuilding model...")
    sam3 = load_sam3()
    model = Phase4AdapterModel(
        sam3, num_classes=NUM_CLASSES, dropout=0.1, dsm_attn_mode=args.dsm_attn_mode,
        checkpoint_attn=args.checkpoint_attn, resolution=args.resolution,
        use_lora=args.use_lora, lora_rank=args.lora_rank, use_prompt=args.use_prompt,
    ).cuda()
