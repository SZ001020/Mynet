#!/usr/bin/env python3
"""Train Plan6 Phase 1.6: MMAdapter + selective attention unfreeze + online crops."""
...
class Window256DatasetDSM(torch.utils.data.Dataset):
    """Fixed validation windows for reproducible comparison across runs."""
...
def main():
    ...
    parser.add_argument("--attn-lr", type=float, default=5e-7)
    ...
    out_dir = os.path.join(args.output, f"plan6_phase1_6_unfreeze_{args.dataset}_{ts}")
    os.makedirs(out_dir, exist_ok=True)
    json.dump(vars(args), open(os.path.join(out_dir, "config.json"), "w"), indent=2)

    print("Plan6 Phase 1.6: MMAdapter + selective attention unfreeze + online crops")
    print(f"  Dataset: {args.dataset}")
    print(f"  Output: {out_dir}")
    print(f"  Unfreeze last {args.unfreeze_layers} attention layers, attn_lr={args.attn_lr}")
    ...
    if args.init_from:
        ...
        print(f"  Missing keys: {len(missing)} (expected for unfrozen attn weights)")
    ...
    attn_params = [p for n, p in model.named_parameters()
                   if p.requires_grad and id(p) not in dsm_param_ids
                   and any(k in n for k in ("qkv", "proj"))
                   and "adapter" not in n]
    attn_param_ids = {id(p) for p in attn_params}

    adapter_params = [p for p in model.parameters()
                      if p.requires_grad and id(p) not in dsm_param_ids
                      and id(p) not in attn_param_ids]

    print(f"  Optim params: adapter={sum(p.numel() for p in adapter_params):,}, "
          f"dsm={sum(p.numel() for p in dsm_params):,}, "
          f"unfrozen_attn={sum(p.numel() for p in attn_params):,}")
