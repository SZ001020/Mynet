#!/usr/bin/env python3
"""
Plan3 Route B: LoRA-SAM3 ViT + UNetFormer Decoder

用法:
  cd /root/Mynet/RS-SAM-p3b
  python train_lora.py --dataset vaihingen --rank 8 --epochs 20 --batch 4
"""
...
from lora_sam3 import LoRASAM3UNetFormer
...
from dataset_adapter import (ISPRSTrainDataset, ISPRSTrainDatasetDSM, DSM_PATTERNS,
                             ...
                             POTSDAM_TRAIN, POTSDAM_VAL, NUM_CLASSES, IGNORE_INDEX)
...
def create_dataloaders(dataset_name, batch_size=4, crop_size=512):
    ...
    train_ds = ISPRSTrainDataset(img_dir, gt_dir, td, isuf, gsuf, crop_size, True)
    val_ds = ISPRSTrainDataset(img_dir, gt_dir, vd, isuf, gsuf, crop_size, False)
...
def validate(model, loader, device):
    ...
    for batch in loader:
        ...
        logits = model(images, dsm) if dsm is not None else model(images)
...
def main():
    parser = argparse.ArgumentParser(description='Plan3 Route B: LoRA-SAM3 ViT + UNetFormer')
    parser.add_argument('--dataset', default='vaihingen', choices=['vaihingen', 'potsdam'])
    parser.add_argument('--rank', type=int, default=8, help='LoRA rank')
    parser.add_argument('--dsm', action='store_true', help='Enable DSM fusion')
    ...
    dsm_tag = '_dsm' if args.dsm else ''
    out_dir = os.path.join(args.output, f'plan3_b_lora_r{args.rank}{dsm_tag}_{ts}')
    os.makedirs(out_dir, exist_ok=True)

    print(f"Plan3 Route B: LoRA-SAM3 + UNetFormer {'+DSM' if args.dsm else ''}")
    ...
    if args.dsm:
        ...
        train_ldr, val_ldr = create_dataloaders(args.dataset, args.batch, args.crop)
    print(f"  Train batches: {len(train_ldr)}")

    # Model
    print("\nBuilding model...")
    sam3 = load_sam3(device)
    model = LoRASAM3UNetFormer(sam3, lora_rank=args.rank, lora_alpha=args.rank * 2,
                               num_classes=NUM_CLASSES, dropout=0.1,
                               use_dsm=args.dsm).cuda()
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for bi, batch in enumerate(train_ldr):
            if args.dsm:
                ...
                images, labels = batch
            images, labels = images.to(device), labels.to(device)
            logits = model(images, dsm if args.dsm else None)
            logits = F.interpolate(logits, labels.shape[-2:], mode='bilinear', align_corners=False)
        ...
        if do_val:
            aAcc, mIoU, per_class = validate(model, val_ldr, device)
        else:
