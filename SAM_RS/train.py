...
if MODE == 'Train':
    ...
    if DATASET == 'Vaihingen':
        best_model = '/root/SSRS/SAM_RS/resultsv/UNetformer_epoch41_0.8248145553437665'
        print(f"Loading best model: {best_model}")
        net.load_state_dict(torch.load(best_model)) 
        net.eval()
        
        # We need the raw probabilities for heatmaps, so we slightly modify the logic here
        # or just use the existing test function and then re-run a simplified version
        # Let's just modify the block to save what we need.
        
        test_images = (1 / 255 * np.asarray(io.imread(DATA_FOLDER.format(id)), dtype='float32') for id in test_ids)
        
        with torch.no_grad():
            for i, (img, id_) in enumerate(zip(test_images, test_ids)):
                ...
                for coords in tqdm(grouper(batch_size, sliding_window(img, step=stride, window_size=window_size)), total=total, leave=False):
                    image_patches = [np.copy(img[x:x + w, y:y + h]).transpose((2, 0, 1)) for x, y, w, h in coords]
                    image_patches = Variable(torch.from_numpy(np.asarray(image_patches)).cuda())
                    outs = net(image_patches)
                    outs = F.softmax(outs, dim=1).data.cpu().numpy()
                    
                    for j in range(len(coords)):
                        x, y, w, h = coords[j]
                        pred[x:x + w, y:y + h] += outs[j].transpose(1, 2, 0)
                
                # Inference map (argmax)
                prediction = np.argmax(pred, axis=-1)
                color_prediction = convert_to_color(prediction)
                save_path_inf = '/root/SSRS/SAM_RS/resultsv/inference_nosam_'+MODEL+'_tile_{}.png'.format(id_)
                io.imsave(save_path_inf, color_prediction)
                
                # Heatmap (Max probability / Confidence)
                # Normalize pred because of potential overlaps
                pred_norm = pred / np.sum(pred, axis=-1, keepdims=True)
                confidence = np.max(pred_norm, axis=-1)
                
                # Save heatmap using a colormap
                heatmap = cm.jet(confidence) # returns RGBA
                heatmap = (heatmap[:, :, :3] * 255).astype(np.uint8)
                save_path_heat = '/root/SSRS/SAM_RS/resultsv/heatmap_nosam_'+MODEL+'_tile_{}.png'.format(id_)
                io.imsave(save_path_heat, heatmap)
                
                print(f"Saved nosam inference to {save_path_inf}")
                print(f"Saved nosam heatmap to {save_path_heat}")
